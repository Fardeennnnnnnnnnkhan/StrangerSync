...
function OrbitItem({ item, index, totalItems, path, itemSize, rotation, progress, fill }) {
  ...
  return (
    <motion.div
      className="absolute will-change-transform select-none transition-all duration-300 hover:scale-125 z-20"
      style={{
        width: itemSize,
        height: itemSize,
...
export default function OrbitImages({
  ...
  const [scale, setScale] = useState( scale => scale || 1);
  ...
  const items = images.map((src, index) => (
    <div key={src + index} className="w-full h-full p-1 group/item transition-all duration-700 hover:scale-115 active:scale-95">
      <div className="w-full h-full rounded-2xl overflow-hidden glass-panel border-white/20 relative shadow-2xl group-hover/item:border-primary/40 transition-colors">
        <img
          src={src}
          alt={`${altPrefix} ${index + 1}`}
          draggable={false}
          className="w-full h-full object-cover opacity-90 group-hover/item:opacity-100 transition-all duration-500 scale-105 group-hover/item:scale-100"
        />
        {/* Glow Overlay */}
        <div className="absolute inset-0 bg-gradient-to-tr from-primary/15 via-transparent to-white/10 opacity-0 group-hover/item:opacity-100 transition-opacity duration-700" />
